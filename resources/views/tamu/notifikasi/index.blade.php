                        ...
                        @if(!$notif->is_read)
                            <div class="flex-shrink-0 self-end md:self-start">
                                <form action="{{ route('tamu.notifikasi.baca', $notif->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" 
                                            class="inline-flex items-center gap-2 px-4 py-1.5 bg-white border border-ant-primary text-ant-primary text-[13px] font-bold rounded-full transition-all duration-300 hover:bg-ant-primary hover:text-white group/btn shadow-sm">
